...
class ResBlock_Cond(EmbedBlock):
    """
    A conditional residual block that can optionally change the number of channels.
    :param channels: the number of input channels.
    :param emb_channels: the number of embedding channels.
    :param dropout: the rate of dropout.
    :param out_channel: if specified, the number of out channels.
    ...
    def __init__(
        ...
        if self.audio_condition_type == True:
    ...
    def _forward(self, x, emb):
        ...
        if self.use_scale_shift_norm:
            out_norm, out_rest = self.out_layers[0], self.out_layers[1:]
            scale, shift = torch.chunk(t_emb_out, 2, dim=1)
            h = out_norm(h) * (1 + scale) + shift
...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        self.attn_res = [image_size // res for res in attn_res]
        self.dropout = dropout
        ...
        self.use_audio_emb = use_audio_emb
        
        if use_audio_emb == False:
            time_cond_embed_dim = inner_channel * 4
            self.cond_embed = nn.Sequential(
                nn.Linear(inner_channel, time_cond_embed_dim),
                SiLU(),
                nn.Linear(time_cond_embed_dim, time_cond_embed_dim),
            )
            #watch here!!!!!
            cond_embed_dim = time_cond_embed_dim
        else:
            audio_cond_embed_dim = 1024
            time_cond_embed_dim = inner_channel * 4
            self.cond_embed = nn.Sequential(
                nn.Linear(inner_channel, time_cond_embed_dim),
                SiLU(),
            ...
            self.audio_cond_embed = nn.Sequential(
                nn.Linear(audio_cond_embed_dim, time_cond_embed_dim),
                SiLU(),
                nn.Linear(time_cond_embed_dim, time_cond_embed_dim),
            )
            ...
            for _ in range(res_blocks):
                layers = [
                    ResBlock_Cond(
                        ...
                        audio_condition_type = audio_condition_type,
                      
                    )
                ]
                ch = int(mult * inner_channel)
                if ds in self.attn_res:
                    layers.append(
                        AttentionBlock(
                            ch,
                            use_checkpoint=use_checkpoint,
    ...
    def forward(self, x, gammas, audio_embed = None):
        """
        ...
        if audio_embed == None:
            ...
            t_emb = self.cond_embed(gamma_embedding(gammas, self.inner_channel))
            #audio_embed = self.audio_cond_embed(audio_embed)
            #emb = torch.cat((emb, audio_embed), 1)
            #emb = emb + audio_embed
            
            a_emb = self.audio_cond_embed(audio_embed)
            emb = (t_emb, a_emb)
