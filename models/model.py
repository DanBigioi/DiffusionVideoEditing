...
class Palette(BaseModel):
    ...
    def set_input(self, data):
        ...
        self.audio = self.set_device(data.get('audio'))
        self.cond_image = self.set_device(data.get('cond_image'))
        self.id_frame = self.set_device(data.get('id_frame'))
        self.gt_image = self.set_device(data.get('gt_image'))
        self.previous_frame = self.set_device(data.get('previous_frame'))
    ...
    def get_current_visuals(self, phase='train'):
        dict = {
            ...
            'previous_frame': (self.previous_frame.detach()[:].float().cpu() + 1) / 2,
            'id_frame': (self.id_frame.detach()[:].float().cpu() + 1) / 2
    ...
    def save_current_results(self):
        ...
        for idx in range(self.batch_size):
            ...
            ret_path.append('Prev_Image_{}'.format(self.path[idx]))
            ret_result.append(self.previous_frame[idx].detach().float().cpu())
            
            ret_path.append('Rand_ID_{}'.format(self.path[idx]))
            ret_result.append(self.id_frame[idx].detach().float().cpu())
    ...
    def train_step(self):
        ...
        for train_data in tqdm.tqdm(self.phase_loader):
            ...
            self.optG.zero_grad()
            loss = self.netG(self.gt_image, self.previous_frame, self.id_frame, self.mask, self.audio)
            loss.backward()
            self.optG.step()
    ...
    def val_step(self):
        ...
        with torch.no_grad():
            for val_data in tqdm.tqdm(self.val_loader):
                ...
                if self.opt['distributed']:
                    if self.task in ['inpainting', 'uncropping']:
                        self.output, self.visuals = self.netG.module.restoration(y_t=self.cond_image,
                    ...
                    if self.task in ['inpainting', 'uncropping']:
                        self.output, self.visuals = self.netG.restoration(y_t=self.cond_image,
                                                                          y_0=self.gt_image, mask=self.mask,
                                                                          previous_frame=self.previous_frame,
                                                                          id_frame = self.id_frame,
                                                                          audio=self.audio,
